/**
 * A company currently maintains two lists of part numbers, where each part number is an integer. 
 * Write a C program that compares these lists of numbers and displays the numbers, if any, that
 * are common to both
...
void binarySearchSetup(int arr1[], int arr2[], int numElem)
{
    ...
    int matchingNums[numElem];
    for(int i = 0; i < numElem; i++)
    {
        if(binarySearch(arr2, arr1[i], 0, numElem - 1))
        {
            matchingNums[matchingNumCount] = arr1[i];
            matchingNumCount++;
        }
    ...
    printArray(matchingNums, matchingNumCount);
